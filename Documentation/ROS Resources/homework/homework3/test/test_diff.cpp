...
{
  vx_actual = 0.9 * vx_actual + 0.1 * msg->twist.linear.x;
  pdot_actual = 0.9 * pdot_actual + 0.1 * msg->twist.angular.z;
  received_twist = true;
}
...
TEST(Homework3Test, forward_reverse_test) {
  ...
  EXPECT_NEAR(0.0, vx_actual, 1e-2);
  EXPECT_NEAR(0.0, pdot_actual, 1e-2);
...
TEST(Homework3Test, mixed_test){
  std::cout << term_color::yellow;
  test_info("Forward speed = 0.7 m/s, steering speed = 0.3 rad/s...");
